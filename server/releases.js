...
export function modelReleased(m, cfg) {
  ...
  return (
    ...
    (cfg.mvpModels || []).includes(m.id) ||
    (isReleased(cfg, "uncensored") && UNCENSORED_MODELS.includes(m.id))
  );
}

// Which updates a request needs, if it isn't plain MVP. A request can need
// more than one (an uncensored chat with web search needs both releases).
