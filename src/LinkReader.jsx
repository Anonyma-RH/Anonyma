...
// bytes and is read here, with the same PDF reader Documents uses.
async function toDocument(link, r) {
  let text = r.text || "",
    words = r.words || 0,
...
export function LinkReaderChips({
  ...
  async function readPage(link) {
    ...
    try {
      ...
      const doc = await toDocument(link, r);
      if (alive.current)
        setDocuments((prev) =>
...
// saved message (read only).
export function LinkCard({ doc, onRemove }) {
