...
export function MemoryPanel({
  ...
  onDelete,
  onDeleteAll,
  // Seed Guard is live. Memory goes with every chat, so a seed phrase or key
  // can only be removed here, never saved anyway.
  seedGuard = false,
}) {
  ...
  const draftSeed = useSeedScan(seedGuard, draft);
  const editSeed = useSeedScan(seedGuard, editing?.text || "");
                ...
                <li key={f.id} className={f.enabled ? "" : "paused"}>
                  {editing?.id === f.id ? (
                    <div className="memory-edit">
                      ...
                      />
                      <SeedGuardNotice hit={editSeed} />
                      <div className="scrolls-item-actions">
                        <button
                          type="button"
                          className="small-button primary"
                          disabled={busy || !!editSeed}
                          onClick={saveEdit}
                        >
                          Save
                        </button>
