...
// Checked against the gateway's published policy: PPQ doesn't store prompts,
// and its "zdr" models run only on endpoints that agreed not to store prompts
// or answers. Kept as one constant so the wording changes in one place.
export const PRIVATE_ROUTE_NOTE =
  "Private mode: this chat isn't saved by ANONYMA, runs only on models whose providers have agreed not to store prompts or answers (zero data retention), and Veil masks your details before sending.";
...
export function PrivateModeToggle({ active, onToggle, disabled }) {
  return (
    <button
      ...
      aria-pressed={active}
      disabled={disabled}
      title="Private mode: zero-data-retention models only, nothing saved, Veil on"
      onClick={onToggle}
    >
...
// anonyma.private ({ privacy, stored }); `masked` is Veil's mask count for
