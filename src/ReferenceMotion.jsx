...
export function Reveal({ children, className = "" }) {
  ...
  useEffect(() => {
    ...
    const covers = split.words.map((word) => {
      ...
      cover.className = "reference-word-cover";
      word.append(cover);
      return cover;
    });
    let motion;
...
export function useHeroMotion(ref) {
  useEffect(() => {
    ...
    const first = createTimeline({
      ...
      .add(node.querySelector(".n-pills"), { y: "50svh", scale: 0 })
      .add(node.querySelector(".n-hero-media"), { y: "-5svh" }, "<<");
    ...
    if (matchMedia("(min-width:940px)").matches)
      second
        .add(node.querySelector(".n-hero-content"), { y: "-20%" })
        .add(
          node.querySelector(".n-hero-media"),
          { translate: "0 -50svh" },
          "<<",
        );
    const fg = createAnimatable(pills, { x: 1200, y: 1200 }),
      bg = createAnimatable(node.querySelector(".n-blurs"), {
        x: 1200,
        y: 1200,
      });
    const move = (e) => {
      if (!matchMedia("(min-width:940px) and (hover:hover)").matches) return;
